...
################################################################################
class Net(nn.Module):
    def __init__(self, conv2_hu, conv3_hu, window):
        ...
        for LEARNING_RATE in LR_LIST:
            ...
            for sec_cnn_hu in SECOND_LAYER_RANGE:
                for thir_cnn_hu in THIRD_LAYER_RANGE:
                    NETWORK = Net(sec_cnn_hu,thir_cnn_hu,window).to(DEVICE)
                    ...
                    #--------Train the model-------------------------------------------------------#
                    ################################################################################
                    FIGURE_NAME = FIGURE_DIR + "{}_1_{}_2_{}_LogLoss.png".format(NETWORK_NAME,str(sec_cnn_hu),str(thir_cnn_hu))
                    TRAIN_MOV_AVG =[]
                    VAL_MOV_AVG = []
                    ...
                    with open(file_name, mode="a+") as output_csv:
                        ...
                        H_P_LIST = [sec_cnn_hu, thir_cnn_hu, window, PADDING_LIST, BATCH_SIZE, LEARNING_RATE, EPOCH_NUM]
                        for DATA_SET in DATA_LIST:
                            print("Testing {} dataset...".format(DATA_SET))
                            RunDataset(DATA_LIST[DATA_SET],DATA_SET,DEVICE,NETWORK,CSV_WRITER,"{}_1_{}_2_{}_ROC".format(NETWORK_NAME,str(sec_cnn_hu),str(thir_cnn_hu)),FIGURE_DIR,TRAIN_TIME,H_P_LIST)
